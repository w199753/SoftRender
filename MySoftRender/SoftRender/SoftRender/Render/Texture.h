...
namespace softRD
{
	...
	public:
		...
		glm::vec4 internal_Sampler2D(const glm::vec2& uv, glm::vec4& res, const T* p)
		{
			...
			{
				if (wrapType == WrapType::Clamp || wrapType == WrapType::Repeat)    //repeat模式的双线性插值懒得写了
				{
					...
					*/
					...
					glm::vec4 top = glm::vec4(0);
					top = fMath::Lerp(col1, col2, (srcx - x1) /dU );
					glm::vec4 bottom = glm::vec4(0);
					bottom = fMath::Lerp(col3, col4, (srcx - x1) / dU);
